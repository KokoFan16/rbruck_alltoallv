		...
		for (int it = 0; it < loopcount; it++) {
			for (int b1 = 1; b1 <= 1; b1 *= 2){
				for (int b2 = 1; b2 <= 1; b2 *= 2){
					double st = MPI_Wtime();
					mpi_errno = twolayer_communicator_linear_s3(ncores, b1, b2, (char*)send_buffer, sendcounts, sdispls, MPI_UNSIGNED_LONG_LONG, (char*)recv_buffer, recvcounts, rdispls, MPI_UNSIGNED_LONG_LONG, MPI_COMM_WORLD);
					double et = MPI_Wtime();
					double total_time = et - st;

					std::cout << rank << " " << mpi_errno << std::endl;
