...
//				// check correctness
//				int error = check_errors(recvcounts, recv_buffer);
//
//				if (error > 0) {
...
//								+ prepSP_time + SP_time;
//
//						std::cout << "[combineAtav] " << nprocs << ", " << n << ", " << bases[i] << ", " << ttime <<
...
//		}
		...
		for (int i = 0; i < basecount; i++) {
			for (int it=0; it < loopcount; it++) {
				...
				double st = MPI_Wtime();
				mpi_errno = twophase_twolayer_rbruck_alltoallv_s2(ncores, bases[i], (char*)send_buffer, sendcounts, sdispls, MPI_UNSIGNED_LONG_LONG, (char*)recv_buffer, recvcounts, rdispls, MPI_UNSIGNED_LONG_LONG, MPI_COMM_WORLD);
				double et = MPI_Wtime();
				double total_time = et - st;
