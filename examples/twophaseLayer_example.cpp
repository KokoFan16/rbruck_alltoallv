	...
	for (int n = 2; n <= 2; n = n * 2) {
		...
		for (int i = 0; i < basecount; i++) {
			for (int it=0; it < loopcount; it++) {
				...
				double st = MPI_Wtime();
				int res = twophase_twolayer_rbruck_alltoallv(ncores, bases[i], (char*)send_buffer, sendcounts, sdispls, MPI_UNSIGNED_LONG_LONG, (char*)recv_buffer, recvcounts, rdispls, MPI_UNSIGNED_LONG_LONG, MPI_COMM_WORLD);
				double et = MPI_Wtime();
				...
				if (rank == 0 && error > 0) {
					std::cout << "[combineAav] base " << bases[i] << " has errors" << std::endl;
				...
				if (warmup == 0) {
					...
					if (total_time == max_time)
						std::cout << "[combineAav] " << nprocs << ", " << n << ", " << bases[i] << ", " << max_time << std::endl;
				}
			}
		}
