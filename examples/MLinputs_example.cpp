...
int run(int loopcount, int ncores, std::vector<int> bases, int warmup, std::vector<int>& sendsarray, std::vector<int>& recvcounts);

int main(int argc, char **argv) {

    if (argc < 5) {
    	std::cout << "Usage: mpirun -n <nprocs> " << argv[0] << " <inputFile> <loop-count> <ncores-per-node> <base-list> " << std::endl;
    	return -1;
    }
    ...
    run(loopCount, ncores, bases, 0, sendsarray, recvcounts);
    ...
    for (int t = 0; t < loopcount; t++) {
    	...
    	if (warmup == 0) {
			...
			if (max_time == comm_time)
				std::cout << "MPI_Alltoallv, " << rank << " " << comm_time << " " << send_tsize << " " <<  recv_tsize << std::endl;
    	}
    }
