...
//     warm-up only
    run_rbruckv(10, ncores, nprocs, bblock1, radix1, bblock2, radix2, xzero, yzero, number, last, 1);
		...
		for (int it = 0; it < loopcount; it++) {
			...
			if (warmup == 0) {
				...
				if (total_time == max_time)
					std::cout << "[MPIAlltoallv] " << nprocs << " " << ncores << " "<<  max_time << std::endl;
			}
		}
...
//					MPI_Abort(MPI_COMM_WORLD, -1);
			...
			if (warmup == 0) {
				...
				if (total_time == max_time) {
					...
					std::cout << "[TTPL_S1] " << nprocs << ", " << ncores << ", " << radix1 << ", " << bblock1 << ", " << ttime << " " << total_count << std::endl;
				}
			}
		}
...
//					MPI_Abort(MPI_COMM_WORLD, -1);
			...
			if (warmup == 0) {
				...
				if (total_time == max_time) {
					...
					std::cout << "[TTPL] " << nprocs << ", " << ncores << ", " << bblock2 << ", " << radix2 << ", " << ttime << " " << total_count << std::endl;
				}
			}
		}
//			}
