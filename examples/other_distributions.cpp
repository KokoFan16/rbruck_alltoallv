...
void creat_normal_distribution_inputs(int* &sendscounts, int* &recvcounts);
...
int main(int argc, char **argv) {
    ...
    if (argc < 10) {
    	std::cout << "Usage: mpirun -n <nprocs> " << argv[0] << " <loop-count> <ncores-per-node> "
    			"<bblock1> <radix1> <bblock1> <radix1> <xzero> <yzero> <number> <last>" << std::endl;
    	return -1;
    }
...
////     warm-up only
//    run_rbruckv(3, ncores, nprocs, bblock1, radix1, bblock2, radix2, xzero, yzero, number, last, 1);
//
//    // actual running
...
//			total_count = ((nprocs - 1) * number + last) * nprocs;
//		}
//
//
...
//							+ prepSP_time + SP_time;
//
//					std::cout << "[TTPL_S1] " << nprocs << ", " << n << ", " << radix1 << ", " << bblock1 << ", " << ttime << " " << total_count << std::endl;
//				}
//			}
//		}
//
...
//							+ prepSP_time + SP_time;
//
//					std::cout << "[TTPL] " << nprocs << ", " << n << ", " << bblock2 << ", " << radix2 << ", " << ttime << " " << total_count << std::endl;
//				}
//			}
//		}
