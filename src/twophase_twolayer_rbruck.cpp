...
int twophase_twolayer_rbruck_alltoallv_s2(int n, int r, char *sendbuf, int *sendcounts,
									   int *sdispls, MPI_Datatype sendtype, char *recvbuf,
									   int *recvcounts, int *rdispls, MPI_Datatype recvtype, MPI_Comm comm)
{
	double st = MPI_Wtime();
...
//	if (rank == 1) {
//		for (int i = 0; i < nprocs; i++) {
//			int d = updated_sentcouts[rotate_index_array[i]];
//			for (int j = 0; j < d; j++){
//				int id = i*max_send_count + j;
//				long long a;
//				memcpy(&a, &extra_buffer[id*sizeof(long long)], sizeof(long long));
//				std::cout << a << std::endl;
//
//			}
//		}
...
//		for (int j = 0; j < n; j++) {
//			int id = i*n + j;
//			int d = updated_sentcouts[rotate_index_array[id]];
//			MPI_Irecv(&recvbuf[nrdisp[nsrc]], nrecv[nsrc]*typesize, MPI_CHAR, src, 0, comm, &req[i]);
//		}
//
////		MPI_Irecv(&recvbuf[nrdisp[nsrc]], nrecv[nsrc]*typesize, MPI_CHAR, src, 0, comm, &req[i]);
